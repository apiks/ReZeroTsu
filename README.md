...
The bot uses **slash commands** (`/`) as the main interface. Use `/help` in the bot to list commands by module. Optional prefix commands (e.g. `.` from config) are supported in DMs only and are less featured. **General:** `help`, `about`, `ping`, `invite`, `avatar`, `roll`, `pick`, `joke`. **Channel:** `prune`. **Server settings:** mod-only mode, moderator roles, bot log channel, donghua in schedule.
...
* **Config:** Copy `config.json.example` to `config.json`. The file must be in the **current working directory** when you run the bot. Set at least `mongo_uri`, `owner_id`, and `prefixes` (used for optional DM prefix commands, e.g. `["."]`). Other options: `mongo_db_timeout`, `playing_msg`, `anime_schedule_api_key`, `user_agent`, `top_gg_token`, `new_guild_log_channel_id`, `log_level`, `log_format`. See the example file for all keys. Anime features (subs, schedule, autopost) require `anime_schedule_api_key` in config (get it from [AnimeSchedule.net](https://animeschedule.net) or leave empty to disable).
